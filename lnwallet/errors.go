...
func ErrFunderBalanceDust(commitFee, funderBalance,
	...
	return ReservationError{
		fmt.Errorf("funder balance too small (%v) with fee=%v bro, "+
			"minimum=%v bro required", funderBalance,
...
func ErrChanReserveTooSmall(reserve, dustLimit btcutil.Amount) ReservationError {
	return ReservationError{
		fmt.Errorf("channel reserve of %v bro is too small, min is %v "+
			"bro", int64(reserve), int64(dustLimit)),
	}
}
...
func ErrChanReserveTooLarge(reserve,
	...
	return ReservationError{
		fmt.Errorf("channel reserve is too large: %v bro, max "+
			"is %v bro", int64(reserve), int64(maxReserve)),
	}
}
