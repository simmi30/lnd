...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	// is denominated in mbro we won't lose precision when storing the
	// bro/kw denominated feerate. Note that we set both the add and remove
...
func (lc *LightningChannel) createCommitDiff(
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		case FeeUpdate:
			...
			// mbro. Since feerates are only denominated in bro/kw,
...
func (lc *LightningChannel) GetDustSum(remote bool) lnwire.MilliSatoshi {
	...
	for e := lc.localUpdateLog.Front(); e != nil; e = e.Next() {
		...
		// If the bronees amount is under the dust limit, add the mbro
...
// channel (either the minimum htlc, or 1 bro).
...
func (lc *LightningChannel) validateFeeRate(feePerKw chainfee.SatPerKWeight) error {
	...
	if newFee > baseBalance {
		return fmt.Errorf("cannot apply fee_update=%v bro/kw, new fee "+
