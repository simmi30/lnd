...
const (
	// FeePerKwFloor is the lowest fee rate in bro/kw that we should use for
	// estimating transaction fees before signing.
	FeePerKwFloor SatPerKWeight = 253

	// AbsoluteFeePerKwFloor is the lowest fee rate in bro/kw of a
	// transaction that we should ever _create_. This is the the equivalent
	// of 1 bro/byte in bro/kw.
	AbsoluteFeePerKwFloor SatPerKWeight = 250
)

// SatPerKVByte represents a fee rate in bro/kb.
...
// FeePerKWeight converts the current fee rate from bro/kb to bro/kw.
...
func (s SatPerKVByte) String() string {
	return fmt.Sprintf("%v bro/kb", int64(s))
}

// SatPerKWeight represents a fee rate in bro/kw.
...
// FeePerKVByte converts the current fee rate from bro/kw to bro/kb.
...
func (s SatPerKWeight) String() string {
	return fmt.Sprintf("%v bro/kw", int64(s))
}
