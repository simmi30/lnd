...
func TestFeeRateTypes(t *testing.T) {
	...
	// Test the conversion from bro/kw to bro/kb.
	for feePerKw := SatPerKWeight(250); feePerKw < 10000; feePerKw += 50 {
		...
		if feePerKB != SatPerKVByte(feePerKw*4) {
			t.Fatalf("expected %d bro/kb, got %d bro/kb when "+
				"converting from %d bro/kw", feePerKw*4,
				feePerKB, feePerKw)
		}
	...
	// Test the conversion from bro/kb to bro/kw.
	for feePerKB := SatPerKVByte(1000); feePerKB < 40000; feePerKB += 1000 {
		...
		if feePerKw != SatPerKWeight(feePerKB/4) {
			t.Fatalf("expected %d bro/kw, got %d bro/kw when "+
				"converting from %d bro/kb", feePerKB/4,
				feePerKw, feePerKB)
		}
