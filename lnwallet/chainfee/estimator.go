...
type BtcdEstimator struct {
	// fallbackFeePerKW is the fall back fee rate in bro/kw that is returned
	// if the fee estimator does not yet have enough data to actually
	// produce fee estimates.
	fallbackFeePerKW SatPerKWeight

	// minFeeManager is used to query the current minimum fee, in bro/kw,
...
// fetchMinRelayFee fetches and returns the minimum relay fee in bro/kb from
// the btcd backend.
func (b *BtcdEstimator) fetchMinRelayFee() (SatPerKWeight, error) {
	...
	// The fee rate is expressed in bro/kb, so we'll manually convert it to
	// our desired bro/kw rate.
...
// confirmation and returns the estimated fee expressed in bro/kw.
//
// NOTE: This method is part of the Estimator interface.
...
// confTarget blocks. The estimate is returned in bro/kw.
func (b *BtcdEstimator) fetchEstimate(confTarget uint32) (SatPerKWeight, error) {
	...
	// Since we use fee rates in bro/kw internally, we'll convert the
	// estimated fee rate from its bro/kb representation to bro/kw.
	satPerKw := SatPerKVByte(satPerKB).FeePerKWeight()

	// Finally, we'll enforce our fee floor.
	if satPerKw < b.minFeeManager.fetchMinFee() {
		log.Debugf("Estimated fee rate of %v bro/kw is too low, "+
			"using fee floor of %v bro/kw instead", satPerKw,
			b.minFeeManager)
		satPerKw = b.minFeeManager.fetchMinFee()
	}

	log.Debugf("Returning %v bro/kw for conf target of %v",
...
type BitcoindEstimator struct {
	// fallbackFeePerKW is the fallback fee rate in bro/kw that is returned
	// if the fee estimator does not yet have enough data to actually
	// produce fee estimates.
	fallbackFeePerKW SatPerKWeight

	// minFeeManager is used to keep track of the minimum fee, in bro/kw,
...
func (b *BitcoindEstimator) fetchMinMempoolFee() (SatPerKWeight, error) {
	...
	// Parse the response to retrieve the min mempool fee in bro/KB.
...
// block targets to fee estimates (in bro per kilovbyte).
...
func (w *WebAPIEstimator) EstimateFeePerKW(numBlocks uint32) (
	...
	log.Debugf("Web API returning %v bro/kw for conf target of %v",
