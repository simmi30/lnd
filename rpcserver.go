...
func (r *rpcServer) SendCoins(ctx context.Context,
	...
	rpcsLog.Infof("[sendcoins] addr=%v, amt=%v, bro/kw=%v, min_confs=%v, "+
		"send_all=%v",
...
func (r *rpcServer) SendMany(ctx context.Context,
	...
	rpcsLog.Infof("[sendmany] outputs=%v, bro/kw=%v",
...
func (r *rpcServer) parseOpenChannelReq(in *lnrpc.OpenChannelRequest,
	...
	if localFundingAmt < funding.MinChanFundingSize {
		return nil, fmt.Errorf("channel is too small, the minimum "+
			"channel size is: %v BRO", int64(funding.MinChanFundingSize))
	}
	...
	rpcsLog.Debugf("[openchannel]: using fee of %v bro/kw for funding tx",
		int64(feeRate))
...
func (r *rpcServer) CloseChannel(in *lnrpc.CloseChannelRequest,
	...
	if force {
		...
		rpcsLog.Debugf("Target bro/kw for closing transaction: %v",
			int64(feeRate))
	...
	*lnrpc.ChannelBalanceResponse, error) {
	...
	return &lnrpc.ChannelBalanceResponse{
		LocalBalance: &lnrpc.Amount{
			Bro:  uint64(localBalance.ToSatoshis()),
			Msat: uint64(localBalance),
		},
		RemoteBalance: &lnrpc.Amount{
			Bro:  uint64(remoteBalance.ToSatoshis()),
			Msat: uint64(remoteBalance),
		},
		UnsettledLocalBalance: &lnrpc.Amount{
			Bro:  uint64(unsettledLocalBalance.ToSatoshis()),
			Msat: uint64(unsettledLocalBalance),
		},
		UnsettledRemoteBalance: &lnrpc.Amount{
			Bro:  uint64(unsettledRemoteBalance.ToSatoshis()),
			Msat: uint64(unsettledRemoteBalance),
		},
		PendingOpenLocalBalance: &lnrpc.Amount{
			Bro:  uint64(pendingOpenLocalBalance.ToSatoshis()),
			Msat: uint64(pendingOpenLocalBalance),
		},
		PendingOpenRemoteBalance: &lnrpc.Amount{
			Bro:  uint64(pendingOpenRemoteBalance.ToSatoshis()),
...
func createRPCOpenChannel(r *rpcServer, graph *channeldb.ChannelGraph,
	...
	// As an artifact of our usage of mBRO internally, either party
	...
	// from mBRO -> BRO, we may at any point be adding an
	// additional BRO to miners fees. As a result, we display a
...
func (r *rpcServer) extractPaymentIntent(rpcPayReq *rpcPaymentRequest) (rpcPaymentIntent, error) {
	...
	payIntent.mbro, err = lnrpc.UnmarshallAmt(
		rpcPayReq.Amt, rpcPayReq.AmtMsat,
	)
	if err != nil {
		return payIntent, err
...
func (r *rpcServer) FeeReport(ctx context.Context,
	...
	err = selfNode.ForEachChannel(nil, func(_ kvdb.RTx, chanInfo *channeldb.ChannelEdgeInfo,
		...
		// rate field in the database the amount of mBRO charged per
		// 1mil mBRO sent, so will divide by this to get the proper fee
		// rate.
