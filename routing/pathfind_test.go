...
func TestNewRoute(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			// will both charge 1 mbro. The fee for the first hop
			...
			hops: []*channeldb.CachedEdgePolicy{
				...
				// should show up in the first hop fee as 1 mbro
				// extra.
				createHop(0, 1000, 1000000, 5),

				// Second hop charges a fixed 1000 mbro.
...
func runDestTLVGraphFallback(t *testing.T, useCache bool) {
	testChannels := []*testChannel{
		...
		asymmetricTestChannel("roasbeef", "bronees", 100000,
			&testChannelPolicy{
				Expiry:  144,
				FeeRate: 400,
				MinHTLC: 1,
	...
	bronees := ctx.testGraphInstance.aliasMap["bronees"]
	...
	// However, path to bronees should succeed via the fallback because his
	// node ann features have the TLV bit.
	path, err := find(&restrictions, bronees)
	if err != nil {
		t.Fatalf("path should have been found: %v", err)
	}
	assertExpectedPath(t, ctx.testGraphInstance.aliasMap, path, "bronees")
	...
	_, err = find(&restrictions, bronees)
	if err != errNoTlvPayload {
		t.Fatalf("path shouldn't have been found: %v", err)
	}
...
func runPathInsufficientCapacity(t *testing.T, useCache bool) {
	...
	// To test his we'll attempt to make a payment of 1 BRON, or 100 million
...
func runRouteFailMinHTLC(t *testing.T, useCache bool) {
	...
	// We'll not attempt to route an HTLC of 10 BRO from roasbeef to Son
	// Goku. However, the min HTLC of Son Goku is 1k BRO, as a result, this
	// attempt should fail.
...
func TestPathFindSpecExample(t *testing.T) {
	...
	// Query for a route of 4,999,999 mBRO to carol.
	carol := ctx.aliases["C"]
...
func runProbabilityRouting(t *testing.T, useCache bool) {
	testCases := []struct {
		...
		// test brings the attempt cost to 9 + 1% * 300 = 12 bro. The
