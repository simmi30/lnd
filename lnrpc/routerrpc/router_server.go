...
func (s *Server) EstimateRouteFee(ctx context.Context,
	...
	// Next, we'll convert the amount in satoshis to mBRO, which are the
	// native unit of LN.
...
// returns the mbro amount and timestamp for it.
...
// getMsatPairValue checks the mbro and bro values set for a pair and ensures
...
func getMsatPairValue(msatValue lnwire.MilliSatoshi,
	...
	// If our mbro value converted to sats equals our bro value, we just
	// return the mbro value, since the values are the same.
	...
	// Likewise, we can just use mbro value if we have no bro value set.
	if satValue == 0 {
		return msatValue, nil
	}
	...
	// set, so we fail.
	return 0, fmt.Errorf("mbro: %v and bro: %v values not equal", msatValue,
		satValue)
}
