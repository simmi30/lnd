...
func TestDust(t *testing.T) {
	...
	// test fee rate, the sweep tx will pay 4860 bro in fees.
	//
	// Create an input so that the output after paying fees is still
	// positive (400 bro), but less than the dust limit (537 bro) for the
...
func TestWalletUtxo(t *testing.T) {
	...
	// fee floor, the sweep tx will pay 439*253/1000 = 111 bro in fees.
	//
	// Create an input so that the output after paying fees is still
	// positive (183 bro), but less than the dust limit (537 bro) for the
	// sweep tx output script (P2WPKH).
	//
...
func TestCpfp(t *testing.T) {
	...
	// Because we sweep at 1000 bro/kw, the parent cannot be paid for. We
	...
	// package, making a total of 1059. At 5000 bro/kw, the required fee for
	// the package is 5295 sats. The parent already paid 900 sats, so there
	// is 4395 bro remaining to be paid. The expected output value is
