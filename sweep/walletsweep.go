...
type FeePreference struct {
	...
	// rate expressed in bro/kw for a particular transaction.
...
// DetermineFeePerKw will determine the fee in bro/kw that should be paid given
// an estimator, a confirmation target, and a manual value for bro/byte. A
...
func DetermineFeePerKw(feeEstimator chainfee.Estimator,
	...
	// If a manual bro/byte fee rate is set, then we'll use that directly.
	// We'll need to convert it to bro/kw as this is what we use
	// internally.
	case feePref.FeeRate != 0:
		feePerKW := feePref.FeeRate
		if feePerKW < chainfee.FeePerKwFloor {
			log.Infof("Manual fee rate input of %d bro/kw is "+
				"too low, using %d bro/kw instead", feePerKW,
